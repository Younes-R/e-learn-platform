import { getSessionsByMonth } from "@/database/dal/teacher";
...
export async function GET(request: NextResponse) {
  ...
  try {
    const res = await getSessionsByMonth("didact@gmail.com", currentMonth);
    res.map((session) => {
  ...
  console.log(currentMonthSessions.get("2023-4-2"));
  console.log(previousMonthSessions.keys());
  console.log(nextMonthSessions.keys());

  return Response.json({
    currentMonthSessions: current ? currentMonthSessions : null,
